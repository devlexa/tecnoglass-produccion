...
public class AuthService : IAuthService
{
    ...
    public async Task<Result<LoginResponse>> LoginAsync(LoginRequest request)
    {
        ...
        if (usuario is null || request.Password != "password")
...
public class OrdenProduccionService : IOrdenProduccionService
{
    ...
    public OrdenProduccionService(OrdenProduccionRepository ordenRepo) => _ordenRepo = ordenRepo;
    ...
    public async Task<Result<OrdenResumenDto>> CrearOrdenAsync(CrearOrdenRequest request, int usuarioId)
    {
        if (request.CantidadVentanas <= 0)
            return Result<OrdenResumenDto>.Fail("La cantidad debe ser mayor a cero.");
        ...
        return detalle is null
            ? Result<OrdenResumenDto>.Fail("Error al recuperar la orden.")
...
public class VentanaService : IVentanaService
{
    ...
    public VentanaService(VentanaRepository ventanaRepo) => _ventanaRepo = ventanaRepo;
...
public class DashboardService : IDashboardService
{
    ...
    public DashboardService(DashboardRepository dashRepo) => _dashRepo = dashRepo;
